...
namespace tcpServer
{
    public class TcpServerConnection
    {
        ...
        //--------------------------------- prueba de captura de ip ---------------------------------------
        public string Capturaripyport()
        {
            string cliente_remoto;
            
            cliente_remoto = m_socket.Client.RemoteEndPoint.ToString();
            return cliente_remoto;
        }

        //--------------------------------- enviar bytes --------------------------------------------------

        public void sendDataByte(byte[] databyte)
        {
            //byte[] array = m_encoding.GetBytes(data);
            byte[] array = databyte;
            lock (messagesToSend)
        ...
        {
            //byte[] array = m_encoding.GetBytes(data);
            //byte[] array = m_encoding.GetString(databyte);
            for (int i = 0; i < datastring.Length; i++)
            {
                byte[] array = m_encoding.GetBytes(datastring[i]);
                lock (messagesToSend)
                {
                    //messagesToSend.Add(array);
                    messagesToSend.Add(array);
